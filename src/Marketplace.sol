...
contract Marketplace is Pausable, Ownable {
    ...
    function listToken(
        ...
        require(
            zangNFTAddress.exists(_tokenId),
            "Marketplace: token does not exist"
        );
        require(
            zangNFTAddress.isApprovedForAll(msg.sender, address(this)),
            "Marketplace: Marketplace contract is not approved"
        );
        require(
            _amount <= zangNFTAddress.balanceOf(msg.sender, _tokenId),
            "Marketplace: not enough tokens to list"
        );
    ...
    function editListingAmount(
        ...
        require(
            listings[_tokenId][_listingId].seller == msg.sender,
            "Marketplace: can only edit own listings"
        );
        require(
            _amount <= zangNFTAddress.balanceOf(msg.sender, _tokenId),
            "Marketplace: not enough tokens to list"
        ...
        require(
            listings[_tokenId][_listingId].amount == _expectedAmount,
            "Marketplace: expected amount does not match"
        );
        ...
        emit TokenListed(
            ...
            _listingId,
            _amount,
            listings[_tokenId][_listingId].price
        );
    }
    ...
    function editListingPrice(
        ...
        emit TokenListed(
            ...
            msg.sender,
            _listingId,
            listings[_tokenId][_listingId].amount,
            _price
        );
    }

    function delistToken(uint256 _tokenId, uint256 _listingId)
        external
        whenNotPaused
    {
        require(
        ...
        // require(_listingId < listingCount[_tokenId], "Marketplace: listing ID out of bounds"); // Opt.
        /*require(
            listings[_tokenId][_listingId].seller != address(0),
            "Marketplace: cannot interact with a delisted listing"
        ); // Opt.*/
        require(
            listings[_tokenId][_listingId].seller == msg.sender,
            "Marketplace: can only remove own listings"
        );
    ...
    function _handleFunds(uint256 _tokenId, address seller) private {
        ...
        uint256 platformFee = (value * zangNFTAddress.platformFeePercentage()) /
            10000;

        uint256 remainder = value - platformFee;

        (address creator, uint256 creatorFee) = zangNFTAddress.royaltyInfo(
            _tokenId,
            remainder
        );
        ...
        (sent, ) = payable(zangNFTAddress.zangCommissionAccount()).call{
            value: platformFee
        }("");
    ...
    function buyToken(
        ...
        require(
            _listingId < listingCount[_tokenId],
            "Marketplace: listing index out of bounds"
        ); // Opt.
        require(
            listings[_tokenId][_listingId].seller != address(0),
            "Marketplace: cannot interact with a delisted listing"
        ...
        require(
            listings[_tokenId][_listingId].seller != msg.sender,
            "Marketplace: cannot buy from yourself"
        );
        require(
            _amount <= listings[_tokenId][_listingId].amount,
            "Marketplace: not enough tokens to buy"
        );
        address seller = listings[_tokenId][_listingId].seller;
        /* If seller transfers tokens "for free", 
        their listing is still active! If they get them back they can still be bought */
        require(
            _amount <= zangNFTAddress.balanceOf(seller, _tokenId),
            "Marketplace: seller does not have enough tokens"
        );
        ...
        require(
            ...
            "Marketplace: price does not match"
        );
        ...
        zangNFTAddress.safeTransferFrom(
            seller,
            msg.sender,
            _tokenId,
            _amount,
